...
with driver.session() as session:
    ...
    #         }
    #         pairs.append(pair)
    #     print("Number of", query["outpair"], "pairs:", len(pairs))
    ...
    for record in results:
        ...
        event_properties = record[f"property_properties"]
        event_name = event_properties["text"]
        event_type = f"{event_properties['type']}"
        if "subtype0" in event_properties:
            event_type = event_type + f"/{event_properties['subtype0']}"
        ...
        pair = {
            "object_type": object_type,
            "object_name": object_name,
            "event_relation": "hasProperty",
            f"property_type": event_type,
            f"property_name": event_name,
            "object_relations": []
        }
    ...
    list_to_json(pairs, f"pathPatterns/test.json")
