...
def create_nodes(tx, entities, unique_entities, tokens, entry_index):
    ...
    for entity in entities:
        ...
        unique_entity_key = (entity_text, entity_type, entry_index)
        ...
        if unique_entity_key not in unique_entities:
            ...
            properties = {"id": entity_id, "text": entity_text, "type": entity_type, "entry_index": entry_index}
            query = f"MERGE (n:{entity_type} {{id: $id, text: $text, type: $type, entry_index: $entry_index}}) "
...
def create_graph(tx, data):
    ...
    for entry_index, entry in enumerate(data):
        ...
        tokens = entry["tokens"]
        current_entities, unique_entities = create_nodes(tx, entities, unique_entities, tokens, entry_index)
